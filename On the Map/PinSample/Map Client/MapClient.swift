        ...
        let task = session.dataTask(with: request) { data, response, error in
            ...
            if let httpResponse = response as? HTTPURLResponse {
                ...
                if httpResponse.statusCode == 401 {
                    DispatchQueue.main.async {
                        completionHandler(nil, NSError(domain: "Invalid Credentials", code: 401, userInfo: [NSLocalizedDescriptionKey: "Invalid username or password"]))
                    }
                return
                }
            }
            ...
            let range = 5..<data.count
            let newData = data.subdata(in: range)
            print("Trimmed Response: \(String(data: newData, encoding: .utf8) ?? "No response data after trim")")
