...
class LoginViewController: UIViewController {
    ...
    // Function to perform the login
     func login(username: String, password: String) {
         let loginRequestBody = UdacityRequest(udacity: LoginRequest(username: username, password: password))
         MapClient.login(responseType: PostUdacityResponse.self, body: loginRequestBody) { response, error in
             if let error = error {
                 DispatchQueue.main.async {
                     self.showAlert(message: "Login failed:Invalid username or password")
                 }
                 return
             }
